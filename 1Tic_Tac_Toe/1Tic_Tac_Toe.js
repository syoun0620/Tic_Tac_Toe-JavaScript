...
tictactoe.getComputerMove = function(board, computerSide) {
  ...
  if (status == true){
  	throw "error for full board";
  }

  var randArray = [0,1,2];
  var rand1 = randArray[Math.floor(Math.random() * randArray.length)];
  var rand2 = randArray[Math.floor(Math.random() * randArray.length)];
